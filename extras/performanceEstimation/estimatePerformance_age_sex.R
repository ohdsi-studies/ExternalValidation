...
resultLoc <- file.path(outputLocation, 'finalResults/Models_age_sex')
      ...
      for(exDatabase in exDatabases){
        ...
        if(!file.exists(file.path(resultLoc, checkName))){
          ...
          externalResults <- tryCatch({
            readRDS(
              file.path( outputLocation, 'simpleModels/Val_age_sex',paste0(internalDatabase,'-',analysis,'-',exDatabase))
            )}, error = function(e){print(e); return(NULL)}
          )
