...
resultLoc <- file.path(outputLocation, 'finalResults/full_model_top100_test')
...
for(internalDatabase in databases){
  ...
  #dataLoc <- paste0('D:/externalValTest/revisedFullModels/halfSample/Data', prefix)
  dataLoc <- file.path(outputLocation, 'Data')
  for(analysis in analyses){
    
    internalPlpData <- tryCatch({
      PatientLevelPrediction::loadPlpData(
        file.path(dataLoc, internalDatabase)
    ...
    covs <- internalResults$covariateSummary %>% 
      mutate(val = abs(.data$covariateValue)) %>%
      dplyr::arrange(-val)
    covs <- covs[1:100,]
      ...
      for(exDatabase in exDatabases){
        ...
        if(!file.exists(file.path(resultLoc, checkName))){
          ...
          if(!is.null(externalPlpData) & !is.null(externalResults)){
            ...
            #  dir.create("D:/externalValTest/estimating", recursive = T)
