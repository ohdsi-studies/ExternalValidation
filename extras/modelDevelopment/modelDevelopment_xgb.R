...
for(i in 1:length(outcomeIds)){
  print(outcomeIds[i])
  resultExists <- file.exists(file.path(outputLocation, 'simpleModels','xgb', 'Models_50', database, 
  ...
  if(!resultExists){
    PatientLevelPrediction::runPlp(
      ...
      saveDirectory = file.path(outputLocation, 'simpleModels','xgb', 'Models_50', database)
    )
  }
}
...
valDatabases <- c() # add val databases

models <- dir(
  file.path(outputLocation, 'simpleModels','xgb', 'Models_50', database), 
...
for(modelLoc in models){
  ...
  for(valDb in valDatabases){
    ...
    saveRDS(
      val, 
      file = file.path(outputLocation, 'simpleModels','xgb', 'Val_50', 
