...
resultLoc <- file.path(outputLocation,'finalResults/Models_rf')
if(!dir.exists(resultLoc)){
  dir.create(resultLoc)
}

for(internalDatabase in databases[-1]){
  exDatabases <- databases[databases != internalDatabase]
  
  
  analyses <- dir(file.path(outputLocation,'SimpleModels','rf', 'Models', internalDatabase))
    ...
    for(analysis in analyses){
      ...
      internalResults <- tryCatch({
        PatientLevelPrediction::loadPlpResult(
          file.path(outputLocation,'SimpleModels','rf', 'Models', internalDatabase,analysis,'plpResult')
        )}, error = function(e){print(e); return(NULL)}
      )
        ...
        for(exDatabase in exDatabases){
          ...
          if(!file.exists(file.path(resultLoc, checkName))){
            ...
            externalResults <- tryCatch({
              readRDS(
                file.path( outputLocation, paste0('SimpleModels/rf/Val'),paste0(internalDatabase,'-',analysis,'-',exDatabase))
              )}, error = function(e){print(e); return(NULL)}
            )
