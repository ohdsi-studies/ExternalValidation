...
resultLoc <- file.path(outputLocation, 'finalResults/Models')
...
for(internalDatabase in databases){
  ...
  analyses <- dir(file.path(outputLocation,'SimpleModels', 'Models', internalDatabase))
    ...
    for(analysis in analyses){
      ...
      internalResults <- tryCatch({
        PatientLevelPrediction::loadPlpResult(
          file.path(outputLocation,'SimpleModels', 'Models', internalDatabase,analysis,'plpResult')
        )}, error = function(e){print(e); return(NULL)}
      )
      ...
      if(!is.null(internalResults) & !is.null(internalPlpData)){
        ...
        covs <- internalResults$covariateSummary %>%
          ...
          filter(abs(.data$covariateValue) != 0) %>% # sig is >=0.1
        ...
        for(exDatabase in exDatabases){
          ...
          if(!file.exists(file.path(resultLoc, checkName))){
            ...
            externalResults <- tryCatch({
              readRDS(
                file.path( outputLocation, paste0('SimpleModels/Val'),paste0(internalDatabase,'-',analysis,'-',exDatabase))
              )}, error = function(e){print(e); return(NULL)}
            )
